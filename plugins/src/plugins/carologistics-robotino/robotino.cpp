...
Robotino::~Robotino()
{
	...
	for (std::list<SimDevice *>::iterator it = devices_list_.begin(); it != devices_list_.end();
	     it++) {
		delete *it;
	}
}
 ...
 */
void
Robotino::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
{
	...
	this->update_connection_ =
	  event::Events::ConnectWorldUpdateBegin(boost::bind(&Robotino::OnUpdate, this, _1));
	...
	std::string infrared_name =
	  model_->GetWorld()->GetName() + "::" + name_ + "::infrared_sensor::link::infrared_puck_sensor";
	devices_list_.push_back(
	  (SimDevice *)new InfraredPuckSensor(model_, node_, sensors::get_sensor(infrared_name.c_str())));
	std::string gripper_laser_left_name =
	  model_->GetWorld()->GetName() + "::" + name_ + "::body::gripper_laser_left";
	std::string gripper_laser_right_name =
	  model_->GetWorld()->GetName() + "::" + name_ + "::body::gripper_laser_right";
	devices_list_.push_back((SimDevice *)new GripperLaserSensor(
	  model_, node_, sensors::get_sensor(gripper_laser_left_name.c_str()), LEFT));
	devices_list_.push_back((SimDevice *)new GripperLaserSensor(
	  model_, node_, sensors::get_sensor(gripper_laser_right_name.c_str()), RIGHT));
 ...
 */
void
Robotino::OnUpdate(const common::UpdateInfo & /*_info*/)
{
	//update devices
	for (std::list<SimDevice *>::iterator it = devices_list_.begin(); it != devices_list_.end();
	     it++) {
