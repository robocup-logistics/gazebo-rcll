 ...
 */
PuckHolder::PuckHolder(physics::ModelPtr model, transport::NodePtr node) : SimDevice(model, node)
{
}
...
void
PuckHolder::create_publishers()
{
	//no publishers
}

void
PuckHolder::create_subscribers()
{
	//no subscribers
}
	...
	if (lin_vel.GetLength() < 0.01 && ang_vel.GetLength() > 0.1) {
		...
		if (!puck_attached_) {
			...
			for (int p = 0; p < NUMBER_PUCKS; p++) {
				...
				double distance = sqrt((puck.x - center_x) * (puck.x - center_x)
				                       + (puck.y - center_y) * (puck.y - center_y));
