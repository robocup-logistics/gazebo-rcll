 ...
 */
void
MpsPlacementPlugin::Load(physics::WorldPtr _world, sdf::ElementPtr /*_sdf*/)
{
	...
	machine_info_sub_ = node_->Subscribe(std::string(TOPIC_MACHINE_INFO),
	                                     &MpsPlacementPlugin::on_machine_info_msg,
	                                     this);
	game_state_sub_ =
	  node_->Subscribe(std::string(TOPIC_GAME_STATE), &MpsPlacementPlugin::on_game_state_msg, this);
 ...
 */
void
MpsPlacementPlugin::on_machine_info_msg(ConstMachineInfoPtr &msg)
{
	...
	if (machines_placed_ || !is_game_started_
	    || world_->GZWRAP_SIM_TIME().Double() < WAIT_TIME_BEFORE_PLACEMENT) {
		return;
	}
	...
	for (int i = 0; i < msg->machines_size(); i++) {
		...
		msgs::Set(spawn_mps_msg.mutable_pose(), ignition::math::Pose3d(coord_x, coord_y, 0, 0, 0, ori));
#else
		msgs::Set(spawn_mps_msg.mutable_pose(), math::Pose(coord_x, coord_y, 0, 0, 0, ori));
#endif
 ...
 */
void
MpsPlacementPlugin::on_game_state_msg(ConstGameStatePtr &msg)
{
...
bool
MpsPlacementPlugin::mps_is_placed(std::string mps_name)
{
...
void
MpsPlacementPlugin::spawn_mps(const gzwrap::Pose3d &spawn_pose, std::string model_name)
{
...
void
MpsPlacementPlugin::remove_existing_mps()
{
