...
/** Enum for the states a light can have
   */
typedef enum LightState { OFF, ON, BLINK } LightState;

/** Enum for team names
   */
typedef enum Team { CYAN, MAGENTA, NIL } Team;
...
public:
	...
	// Setter
	void set_light_state(MachineName machine, LightState red, LightState yellow, LightState green);
	void set_light_state(std::string machine, LightState red, LightState yellow, LightState green);
